...
- [ ] **Unassigned-records note** (ADR 0021): some records can't be resolved to a county and the producer leaves their county NA (honest "unassigned"). Set geo level to **Census State** (or County) and select **Maryland** — a bare "Baltimore" can't be split into city vs county, so a second amber note appears above the chart: "Excluded from the geographic breakdown — records with no assigned county: MD (…)." This is expected, **not** a bug. National/Region views never show this note. At Metro/Micro Area level the note instead reads "records in no metro/micro area (rural or unmapped)".
- [ ] **Connecticut planning regions** (ADR 0023, vintage v2026.08): set geo level to **Census County**, pick **Connecticut** as the state — the County picker offers **9 planning regions** (Capitol / Greater Bridgeport / Lower Connecticut River Valley / Naugatuck Valley / Northeastern Connecticut / Northwest Hills / South Central Connecticut / Southeastern Connecticut / Western Connecticut), **not** the 8 retired counties (Fairfield, Hartford, …). CT now also carries Metro/Micro values. Any old bookmark with a retired-CT-county filter no longer matches — that's expected, not a bug.
- [ ] **CT residual unassigned (expected, tiny):** on the **Numbers** panel with the default date range, selecting CT at Census State level shows a small "records with no assigned county: CT (2)" note. That is **correct** — 2 off-grid orgs (Size 0, no coordinate, tax years 2025-2026) remain honestly NA; CT coverage is 99.9996%, not literally 100%. The note only appears on Numbers and only when the date range includes 2025-2026 (narrow to ≤2024 and it disappears). The pre-v2026.08 behavior — *thousands* of CT records unassigned — is what's fixed; a residual "(2)" is not a regression.
