...
#'
#' For each known filter key, sets `output$validation_<key>` to either a
#' styled message div (when an error is present) or NULL (clears any prior
#' message). Always touches every slot so stale errors from a previous
#' click don't linger.
#'
#' @param errors Named list from `validate_inputs()$errors`. Keys: geo,
#'   subsector, size.
